...
export const useAdminStore = defineStore('admin', () => {
  ...
  async function fetchStats() {
    const res = await frappeCall(
      'krcs_internship.krcs_internship.api.get_dashboard_stats'
    )
  ...
  async function fetchCharts() {
    const [deptRes, uniRes] = await Promise.all([
      frappeCall('krcs_internship.krcs_internship.api.get_applications_by_department'),
      frappeCall('krcs_internship.krcs_internship.api.get_applications_by_university')
    ])
  ...
  async function createPosting(payload) {
    const res = await frappeCall(
      'krcs_internship.krcs_internship.api.create_posting',
      payload
    )
    return res.message
  }
  ...
  async function updatePosting(name, payload) {
    return frappeCall(
      'krcs_internship.krcs_internship.api.update_posting',
      { name, ...payload }
    )
  }
  ...
  async function deletePosting(name) {
    return frappeCall(
      'krcs_internship.krcs_internship.api.delete_posting',
      { name }
    )
  }
