  ...
  <div>
    ...
    <form @submit.prevent="handleSubmit" class="form-card">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select v-model="form.department">
            ...
            <option v-for="d in depts" :key="d.name" :value="d.name">{{ d.department || d.name }}</option>
          </select>
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group" v-if="form.stipend_type === 'Paid'">
          <label>Monthly Amount (KES)</label>
          <input v-model.number="form.stipend_amount" type="number" min="0" />
        </div>
        <div class="form-group">
          <label>Status</label>
      ...
      <div class="form-group">
        ...
        <button type="button" class="btn-add-line" @click="responsibilityLines.push('')">+ Add</button>
      </div>
      ...
      <div class="form-group">
        ...
        <button type="button" class="btn-add-line" @click="requirementLines.push('')">+ Add</button>
      </div>
...
onMounted(async () => {
  const res = await store.fetchDepartments()
  depts.value = res

  if (isEdit.value) {
    const posting = await store.fetchPosting(route.params.id)
    if (posting) {
      Object.assign(form.value, posting)
...
.skill-tag button { background: none; border: none; cursor: pointer; color: #9ca3af; font-size: .8rem; padding: 0; line-height: 1; }
...
.form-error { color: #cc0000; font-size: .82rem; margin-top: .5rem; }
</style>